...
VARIANT_HINTS = {
    ...
    "ZOTAC": ["AMP EXTREME INFINITY", "AMP EXTREME", "TWIN EDGE", "TRINITY WHITE", "TRINITY",
              "SOLID", "SOLO", "AMP"],
    "PNY": ["XLR8 GAMING VERTO EPIC-X RGB", "EPIC-X RGB", "VERTO", "XLR8", "DUAL FAN", "SINGLE FAN"],
    ...
    "EVGA": ["FTW3 ULTRA GAMING", "FTW3", "XC3", "XC", "BLACK EDITION", "GAMING"],
...
BUNDLE_KEYWORDS = [
    "GAMING PC", "PC GAMER", "GAMING DESKTOP", "DESKTOP COMPUTER",
    "MINI PC", "MINI COMPUTADORA", "GAMING LAPTOP", "NOTEBOOK",
    "COMPUTADORA DE ESCRITORIO", "GAMING COMPUTER", "EGPU",
]

CPU_PATTERN = r'\b(?:RYZEN\s*\d\s*\d{3,5}[A-Z]*|CORE\s*I[3579]\b|I[3579]-\d{3,5}[A-Z]*)\b'


def is_bundle_listing(title):
    if any(kw in title for kw in BUNDLE_KEYWORDS):
        return True
    if re.search(CPU_PATTERN, title):
        return True
    return False
...
def normalize_title(title):
    ...
    title = title.replace("'", "")
    title = title.replace("-", " ")
    title = title.replace("PERFIL BAJO", "LOW PROFILE")
...
def mark_skipped(listingid, reason):
    try:
        cursor.execute("""
            UPDATE public.listing
            SET parsed = true, parsedat = NOW()
        ...
        """, (listingid,))
        conn.commit()
        print(f"[yellow]Skipped listingid {listingid} ({reason})[/yellow]")
    except Exception as e:
        conn.rollback()
        print(f"[red]Failed to mark skipped listingid {listingid}: {e}[/red]")


cursor.execute("""
    SELECT listingid, storetitle
    FROM listing
    WHERE parsed IS NOT TRUE AND storeid = 8
    ORDER BY listingid
""")
...
for listingid, storetitle in rows:
    ...
    if parsed["skip"]:
        reason = "bundle/non-standalone" if is_bundle_listing(normalize_title(title)) else "unrecognized brand"
        mark_skipped(listingid, reason)
        continue
